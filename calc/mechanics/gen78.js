...
function calculateSMSS(gen, attacker, defender, move, field) {
    ...
    if ((attacker.hasAbility('Triage') && move.drain) ||
        ...
        (attacker.hasAbility('Blazing Soul') &&
            move.hasType('Fire') &&
            attacker.curHP() === attacker.maxHP())) {
        move.priority = 1;
        desc.attackerAbility = attacker.ability;
...
function calculateBasePowerSMSS(gen, attacker, defender, move, field, hasAteAbilityTypeChange, desc) {
    ...
    switch (move.name) {
        ...
        case 'Expanding Force':
            var isTerrainBoosted = (0, util_2.isGrounded)(attacker, field) && field.hasTerrain('Psychic');
        ...
        case 'Misty Explosion':
            ...
            break;
        case 'Rising Voltage':
            basePower = (0, util_2.isGrounded)(defender, field) && field.hasTerrain('Electric') ? move.bp * 2 : move.bp;
            desc.moveBP = basePower;
            break;
        case 'Gyro Ball':
        ...
        case 'Triple Axel':
            ...
            break;
        case 'Lash Out':
            basePower = move.bp * ((0, util_2.countBoosts)(gen, attacker.boosts) < 0 ? 2 : 1);
            desc.moveBP = basePower;
            break;
        case 'Crush Grip':
        case 'Wring Out':
...
function calculateBPModsSMSS(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    var resistedKnockOffDamage = !defender.item ||
        ...
        (defender.named('Dialga', 'Dialga-Primal') && defender.hasItem('Adamant Orb')) ||
    ...
    if (gen.num === 7) {
        var bp = (0, util_2.pokeRound)((basePower * (0, util_2.chainMods)(bpMods)) / 4096);
...
function calculateFinalModsSMSS(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness) {
    ...
    if (defender.hasAbility('Fluffy') && move.flags.contact && !attacker.hasAbility('Long Reach')) {
        ...
        (defender.hasAbility('Primal Armor') && typeEffectiveness > 1)) {
        finalMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
